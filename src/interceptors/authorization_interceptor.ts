...
type AuthorizationInterceptorPositionalParameters = Pick<AuthorizationInterceptor, 'parameters'> & {
    header?: string;
    scheme?: string;
};
...
export class AuthorizationInterceptor extends RequestInterceptor {
    ...
    constructor({ header, scheme, parameters }: AuthorizationInterceptorPositionalParameters) {
        super();
