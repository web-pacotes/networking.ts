...
type NoInternetConnectionErrorPositionalProperties = Omit<
	HttpRequestErrorPositionalProperties,
	'cause'
> & { cause?: string };
type TimeoutErrorPositionalProperties = Omit<
	HttpRequestErrorPositionalProperties,
	'cause'
> & { cause?: string; timeoutMS: number };
type UnknownErrorPositionalProperties = Omit<
	HttpRequestErrorPositionalProperties,
	'cause'
> & { cause?: string };
...
export class TimeoutError extends HttpRequestError {
	...
	constructor({
		...
		timeoutMS
	}: TimeoutErrorPositionalProperties) {
		super({ cause: cause });
